...
#Memory field (bound between 0 and 1) increases in time as long as spins (pairwise) are in an energetically unfavorable configuration (i.e. s_1 = -1, s_2 = -1, while J_{1 2} = -1 (antiferromagnetic))
def spin_dynamics(general_params, check_initialization, spin_tracker, instance_num):
    ...
    #Initializes the spin and memory coupling arrays (J_{i j} and x_{i j})
    #Sets J^{up}_{i j} and J^{right}_{i j} to be random arrays of -1s, and 1s (each INTERACTION is either ferromagnetic or antiferromagnetic)
    #J^{down}_{i j} and J^{left}_{i j} are determined by shifting the J^{up} and J^{right} arrays up and to the right, respectively (to preserve symmetry of interactions)
    #Sets x_{i j} to a constant array of xinis (symmetry constraint is automatically enforced since all entries are xini)
    #This approach makes both J_{i j} and x_{i j} ONLY non-zero for nearest-neighbor interactions
    ...
    #Creates the file that this instance's spin and memory dynamics will be written to and catalogues the initial spins
    spin_file = open(param_string + '/' + str(instance_num) + '.txt', 'a')
    memory_up_file = open(param_string + '/' + str(instance_num) + 'MemUp.txt', 'a')
    memory_right_file = open(param_string + '/' + str(instance_num) + 'MemRight.txt', 'a')
    for i in range(sz[0]):
        for j in range(sz[1]):
            spin_file.write(str(spin[i][j]) + ',') #writes the initialized spin values to this file
            memory_up_file.write(str(memory_up[i][j]) + ',') #writes the initialized memory values to these file (down and left are redundant)
            memory_right_file.write(str(memory_right[i][j]) + ',')
    spin_file.write('\n')
    memory_up_file.write('\n')
    memory_right_file.write('\n')
    ...
    while np.round(t, 4) < T:
        ...
        #Saves most recent spin and memory values to a file within the corresponding (parameter-based) directory and to an accessible array
        for i in range(sz[0]):
            for j in range(sz[1]):
                spin_file.write(str(spin[i][j]) + ',')
                memory_up_file.write(str(memory_up[i][j]) + ',')
                memory_right_file.write(str(memory_right[i][j]) + ',')
        spin_file.write('\n')
        memory_up_file.write('\n')
        memory_right_file.write('\n')
    ...
    #Writes the timestep at which the dynamics stopped to the corresponding spin and memory files (may be t < T if dynamics stopped early due to "interesting spins" feature)
    spin_file.write(str(t))
    spin_file.close()
    memory_up_file.write(str(t))
    memory_up_file.close()
    memory_right_file.write(str(t))
    memory_right_file.close()
